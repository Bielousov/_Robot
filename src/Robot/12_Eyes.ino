...
const uint8_t EYES_OPEN_ANIMATION_SEQUENCE[] = {5, 4, 3, 2, 1};
const uint8_t EYES_CLOSE_ANIMATION_SEQUENCE[] = {1, 2, 3, 4, 5};
...
Thread eyesAnimationThread = Thread(onEyesAnimation, 1000 / EYES_ANIMATION_FPS);
...
bool applyLashMask(uint8_t x, uint8_t y, bool ledPixel) {
  const byte* eyeBlinkAnimationFrame = eyeBlinkAnimationBitmap[State.Eyes.lashesPosition - 1];
  bool lashMaskPixel = loadBitmapPixel(eyeBlinkAnimationFrame, x, y);
  bool lashMaskTopPixel = y > 0 && loadBitmapPixel(eyeBlinkAnimationFrame, x, y - 1);
  bool lashMaskBottomPixel = y < BITMAP_SIZE - 1 && loadBitmapPixel(eyeBlinkAnimationFrame, x, y + 1);

  bool isMaskEdge = lashMaskTopPixel != lashMaskPixel || lashMaskBottomPixel != lashMaskPixel;
  return !lashMaskPixel || (State.Eyes.lashesPosition > 1 && isMaskEdge)  ? lashMaskPixel : ledPixel;
}
...
void drawEyes(byte* bitmap) {  
  for (uint8_t x = 0; x < BITMAP_SIZE; x++) {
    for (uint8_t y = 0; y < BITMAP_SIZE; y++) {
      // Get each led pixel and apply masks to it
      bool ledPixel = bitRead(bitmap[y], x);

      // Rotate matrix CCW
      lmd.setPixel(BITMAP_SIZE - y - 1, x, ledPixel);
    }
  }
...
void onEyesAnimation() {
  ...
  byte* frameBitmap = animationBuffer.popFrame();
